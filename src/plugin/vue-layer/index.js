...
//点触事件
    ...
    Layer.prototype.view = function(){
        ...
        if(config.type === 3){
            config.shade = false;
            config.content = '<i></i><i class="layui-m-layerload layui-m-myload "></i><i></i><p>'+ (config.content||'') +'</p>';
        }
        ...
        if(!config.type || config.type === 2){
            ...
            if(dialen >= 1){
                win.layer.close(dialogs[0].getAttribute('index'))
            }
        }
    ...
    Layer.prototype.action = function(config, elem){
        ...
        var btn = function(){
            ...
            if(type == 0){
                ...
                config.yes ? config.yes(that.index) : win.layer.close(that.index);
            }
        };
        if(config.btn){
        ...
        if(config.shade && config.shadeClose){
            ...
            ready.touch(shade, function(){
                win.layer.close(that.index, config.end);
            });
        }
    ...
    win.layer = {
        ...
        closeAll: function(){
            ...
            for(var i = 0, len = boxs.length; i < len; i++){
                win.layer.close((boxs[0].getAttribute('index')|0));
            }
        }
    };
